...
export class App extends Component {
  constructor(props) {
    ...
    this._onContextMenuRemoveButtonPressed = this._onContextMenuRemoveButtonPressed.bind(
      this,
    );
        ...
        if (!retDict.success) {
          if (retDict.errorCode === ViroConstants.RECORD_ERROR_NO_PERMISSION) {
            this._displayVideoRecordAlert(
              'Screenshot Error',
              'Please allow camera permissions!' + retDict.errorCode,
            );
          }
        }
    ...
    this._arNavigator._stopVideoRecording().then((retDict) => {
      if (!retDict.success) {
        if (retDict.errorCode === ViroConstants.RECORD_ERROR_NO_PERMISSION) {
          this._displayVideoRecordAlert(
            'Recording Error',
            'Please allow camera record permissions!' + retDict.errorCode,
          );
        }
      }
      this.setState({
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    dispatchSwitchListMode: (listMode, listTitle) =>
      dispatch(switchListMode(listMode, listTitle)),
