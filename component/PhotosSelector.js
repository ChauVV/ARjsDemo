...
export class PhotosSelector extends Component {
  ...
  render() {
    console.log('this.state.dataSource 11: ', this.state.dataSource);
    return (
      <View style={[localStyles.outerContainer, this.props.style]}>
        <FlatList
          style={localStyles.scrollView}
          data={this.state.dataSource}
          keyExtractor={(item) => item.icon_img}
          renderItem={this._getRow}
  ...
  _getRow(data, sectionId, rowIndex) {
    if (
      rowIndex === this.state.dataSource.length - 1 &&
  ...
  _updatePhotoSelection(row, column) {
    let previousSelection = this.refList[
      this._getRefKey(this.selectedRow, this.selectedColumn)
    ];
